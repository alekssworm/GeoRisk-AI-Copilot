...
2. **RAG pipeline** in `rag/`
   ...
   - Searches three shelves, promoting retrieved passages while retaining stable
     references on the bottom; expands retrieval if the upper shelves lack evidence.
   - Persists fitted vectors and decaying popularity through atomic, locked updates.
...
- Uploaded PDFs are chunked into retrievable passages. RAG responses include
  source labels, page numbers, retrieval scores, grounded text snippets, and a
  retrieval trace showing which shelves were visited and how many rows were scored.

The shelf policy, migration, and search tradeoffs are described in
[RAG shelves](rag_shelves.md).
